...
def main_feddr(args):
    ...
    for idx in range(args.num_users):
        ...
        for i in range(args.num_users):
            tmp = partitions_train_stat.get(i)
            arr = []
            for j in range(class_num):
                arr.append(tmp.setdefault(j, 0))
            net_cls_counts_npy.append(arr)
        net_cls_counts_npy = np.array(net_cls_counts_npy)

        clients.append(Client_Fed_Scene(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep,
                                        args.lr, args.momentum, args.device, train_dl_local, test_dl_local,
                                        num_classes=class_num, num_clients=args.num_users,
                                        client_pred=args.client_pred, net_cls_counts_npy=net_cls_counts_npy))
    ...
    for iteration in range(args.rounds):
        ...
        for idx in idxs_users:
            ...
            loss = clients[idx].train(idx, is_print=False, args=args)
        ...
        print_flag = False
        if iteration + 1 in [int(0.10 * args.rounds), int(0.25 * args.rounds), int(0.5 * args.rounds),
                             int(0.8 * args.rounds)]:
            print_flag = True
        ...
        if print_flag:
            print('*' * 25)
            print(
                f'Check Point @ Round {iteration + 1} --------- {int((iteration + 1) / args.rounds * 100)}% Completed')
            temp_acc = []
            temp_best_acc = []
        ...
        scores.append(('{}' + '\t{:.4f}' * 3).format(iteration, loss_avg, acc, best_acc))

        model_filename = 'checkpoint_%03d.pth.tar' % iteration
        save_checkpoint({
            'round': iteration,
            'arch': args.model,
    ...
    for idx in range(num_users_FL):
        clients[idx].set_state_dict(copy.deepcopy(w_glob))
        loss = clients[idx].test(is_print=False)
    ...
    if args.new_comer:
        ...
        for iteration in range(args.ft_epoch):
            ...
            if iteration % 5 == 0:
                ...
                for idx in range(num_users_FL, args.num_users):
                    print(
                        f'Client {idx:3d}, current_acc {new_comer_acc[idx][-1]:3.2f}, best_acc {np.max(new_comer_acc[idx]):3.2f}')
...
def run_feddr(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        final_glob, avg_final_glob, best_glob, avg_final_local, avg_best_local, \
            fedprox_ft_local, duration, f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6, ff7 = main_feddr(args)
